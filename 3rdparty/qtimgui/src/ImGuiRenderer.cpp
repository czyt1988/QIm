...
const QHash< int, ImGuiKey > KEY_MAP = { { Qt::Key_Tab, ImGuiKey::ImGuiKey_Tab },
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++) {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback) {
                ...
                glScissor((int)(clip_min.x / sx),
                          (int)(fb_height - clip_max.y / sy),
                          (int)((clip_max.x - clip_min.x) / sx),
                          (int)((clip_max.y - clip_min.y) / sy));
...
void ImGuiRenderer::newFrame()
{
    ...
    if (m_window->isActive()) {
        ...
        io.MousePos      = ImVec2(
            pos.x(), pos.y());  // Mouse position in screen coordinates (set to -1,-1 if no mouse / on another screen, etc.)
    } else {
...
ImGuiRenderer::ImGuiRenderer() : g_ctx(nullptr)
{
}
