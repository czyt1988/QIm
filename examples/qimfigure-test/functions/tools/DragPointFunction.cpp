    ...
    : TestFunction(parent)
{
    ...
    flagsReg.category = tr("DragPoint");
...
void DragPointFunction::setId(int id)
{
    if (m_id != id) {
        m_id = id;
        Q_EMIT idChanged(id);
        if (m_dragPointNode) {
            m_dragPointNode->setId(id);
        }
    }
}

void DragPointFunction::setFlags(int flags)
{
    if (m_flags != flags) {
        m_flags = flags;
        Q_EMIT flagsChanged(flags);
        if (m_dragPointNode) {
            m_dragPointNode->setFlags(flags);
        }
    }
}
